...
def main(args):
    ...
    final = (0.3170226961630325, 0.5777994301035916, 0.22498384888989406, -0.2684466378799872, -0.3681553890925539, -1.25, 0.0)

    desired_pose = rest
    cocontraction = (0.6, 0.6, 0.6, 0.6, 0.6)
    ...
    while True:
        ...
        if do_shake_hand:
            if time_counter < 60:
                ...
                cocontraction = (0.6, 0.6, 0.85, 0.6, 0.2)
                desired_pose = mid
                r.sleep()
            else:
                if time_counter < 250:
                    print "Moving, second step"
                    cocontraction = (0.5, 0.4, 0.6, 0.55, 0.2)
                    desired_pose = final
                    r.sleep()
                else:
                    print "Waiting..."
                    ...
                    if time_counter < 1150:
                        if not hand_is_closed:
                            if hand_shake.havePersistentTouch():
                                ...
                                #elbow_waiting = gummi.elbow.angle.getEncoder() #TODO
                                hand_is_closed = True
                                time_counter = 750
                        else:
                            elbow = elbow_waiting + width/2 * math.sin(frequency * time_counter/60.0)
                            #gummi.elbow.servoTo(elbow, 0.3) #TODO
                    else:
